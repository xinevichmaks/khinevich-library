...
import { Card, Modal, Avatar, T, sans, btn, input } from "../ui.jsx";
...
export default function Useful() {
  ...
  const save = async () => {
    ...
    if (!vid || !title) return;
    await addItem("useful", { vid, title, channel: "Khinevich Library" });
    setAdd(false); setUrl(""); setTitle("");
  };
          ...
          <div key={v.id} onClick={() => setOpen(v)} style={{ cursor: "pointer" }}>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 10 }}>
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ font: `13px ${sans}`, color: T.faint, marginTop: 3 }}>{v.channel}</div>
              </div>
            </div>
          </div>
        ))}
